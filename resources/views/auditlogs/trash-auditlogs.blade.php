                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Time</th><th>Operator</th><th>Resource</th><th>Ip</th><th>AuditType</th><th>Level</th><th>Result</th><th>Content</th><th>Label</th><th>OldValue</th><th>NewValue</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($auditlogs as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->time }}</td><td>{{ $item->operator }}</td><td>{{ $item->resource }}</td><td>{{ $item->ip }}</td><td>{{ $item->auditType }}</td><td>{{ $item->level }}</td><td>{{ $item->result }}</td><td>{{ $item->content }}</td><td>{{ $item->label }}</td><td>{{ $item->oldValue }}</td><td>{{ $item->newValue }}</td>
                                <td>
                                    <a href='{{ route('auditlogs.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/auditlogs-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
