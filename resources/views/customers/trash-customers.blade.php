            ...
            <div class='row'>
                <div class='col-lg-4 col-md-4 col-sm-12 mt-2'>
                    <div class='row'>
                        ...
                        <div class='col-8'>
                            <form action='{{ url('/customers-paginate') }}' method='get'>
                ...
                <table class='table table-striped'>
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>CustomerId</th><th>Name</th><th>Description</th><th>Users_id</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    ...
                    <tbody>
                        @forelse($customers as $item)
                            <tr>
                                ...
                                <td>{{ $item->id }}</td>
                                <td>{{ $item->customerId }}</td><td>{{ $item->name }}</td><td>{{ $item->description }}</td><td>{{ $item->users_id }}</td>
                                <td>
                                    <a href='{{ route('customers.restore', $item->id) }}'><i class='fas fa-recycle text-info'></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.bulk-restore').click(function () {
                ...
                $.post('/customers-restore-all-bulk-data', {
                    ids: array,
                    _token: $("meta[name='csrf-token']").attr('content')
                }, function (res) {
                    console.log(res)
